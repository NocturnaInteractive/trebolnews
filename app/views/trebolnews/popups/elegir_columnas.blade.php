<script>
$(function() {

    $('#frm_importar_lista #saveForm').on('click', importar_lista_handler);

    function importar_lista_handler(e) {
        e.preventDefault();

        $('#frm_importar_lista #saveForm').on('click', function(e){
            e.preventDefault();
        });

        $('#frm_importar_lista').ajaxSubmit({
            data: {
                id_lista: $('#id_lista').val()
            },
            success: function(data) {
                if(data.status == 'ok') {
                    noty({
                        text: data.mensaje,
                        layout: 'topCenter',
                        timeout: 3000,
                        callback: {
                            afterClose: function() {
                                location.reload();
                            }
                        }
                    });
                } else {
                    notys(data.validator);
                }
            },
            complete: function() {
                $('#frm_importar_lista #saveForm').one('click', importar_lista_handler);
            }
        });
    }

});
</script>

<div id="bg_popup">
    <div id="container_popup" class="popup_listasuscriptores">
        <div id="encabeezado">
            <h3>Asigne un campo a cada columna</h3>
            <a href="#" id="cerrar_popup"><img src="{{ asset('internas/imagenes/cerrar_popup.png') }}" alt="Cerrar Ventana" width="18" height="18"></a>
            <div class="cleaner"></div>
        </div><!--encabeezado-->
        <div id="info_popup">
            <form class="formpopup" id="frm_importar_lista" action="{{ action('ListaController@do_import') }}" method="post">
                <input type="hidden" name="id_lista" value="{{ $lista }}" />
                <input type="hidden" name="archivo" value="{{ $archivo }}" />
                <p>
                    <label for="nombre">Nombre:</label>
                    <select name="nombre">
                        <option value="">---</option>
                        @foreach($columnas as $columna)
                        <option name="{{ $columna }}">{{ $columna }}</option>
                        @endforeach
                    </select>
                </p>

                <p>
                    <label for="apellido">Apellido:</label>
                    <select name="apellido">
                        <option value="">---</option>
                        @foreach($columnas as $columna)
                        <option name="{{ $columna }}">{{ $columna }}</option>
                        @endforeach
                    </select>
                </p>

                <p>
                    <label for="email">Email:</label>
                    <select name="email">
                        <option value="">---</option>
                        @foreach($columnas as $columna)
                        <option name="{{ $columna }}">{{ $columna }}</option>
                        @endforeach
                    </select>
                </p>

                <p>
                    <label for="puesto">Puesto:</label>
                    <select name="puesto">
                        <option value="">---</option>
                        @foreach($columnas as $columna)
                        <option name="{{ $columna }}">{{ $columna }}</option>
                        @endforeach
                    </select>
                </p>

                <p>
                    <label for="empresa">Empresa:</label>
                    <select name="empresa">
                        <option value="">---</option>
                        @foreach($columnas as $columna)
                        <option name="{{ $columna }}">{{ $columna }}</option>
                        @endforeach
                    </select>
                </p>

                <p>
                    <label for="pais">País:</label>
                    <select name="pais">
                        <option value="">---</option>
                        @foreach($columnas as $columna)
                        <option name="{{ $columna }}">{{ $columna }}</option>
                        @endforeach
                    </select>
                </p>

                <div id="botones_popup">
                    <input type="button" value="SUBIR" name="submit" id="saveForm" />
                    <!-- <input class="btn" id="borrar" type="reset" value="BORRAR" name="Enviar2" /> -->
                    <div class="cleaner"></div>
                </div>
            </form>
        </div><!--info_popup-->
    </div><!--container_popup-->
</div><!--bg_popup-->
